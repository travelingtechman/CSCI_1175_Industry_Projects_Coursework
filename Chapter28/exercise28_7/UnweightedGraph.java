...
public class UnweightedGraph<V> implements Graph<V> {
  ...
  public List<Integer> getACycle(){
		ArrayList<Integer> allVertices = new ArrayList<>();  //		    create a list with all the vertices
		for(int i = 0; i < vertices.size(); i++) {
			allVertices.add((i))  ;
		}
		...
		int[] array = new int[vertices.size()]; // create an int array to determine parent of vertex (-1 by default)
		for(int i = 0; i < array.length; i++) {
			array[i] = -1;
		} 
		while(allVertices.size() > 0 ) { //	 while (there are unvisited vertices) { // check list size for this 
			...
			while(!stack.isEmpty()) { //	while (the stack is not empty) { 
				...
				if(neighbors.get(x).size() != 0) {//	if (x has no neighbors) {
					...
					for(int i = 0; i < neighbors.get(x).size(); i++) {// loop through all neighbors of x with loop variable i {
						...
						else if(e.v != array[x]) {		//else if (e.v is not x's parent) {
							ArrayList<Integer> arrayList = new ArrayList<>();	//create an Integer ArrayList;
							arrayList.add(e.v);//add e.v to list;
							while (x != e.v && x != -1) {
								arrayList.add(x);//add x to list;
								x = array[x];//	x is parent of x;
							}
							return arrayList;
						}
						else { 
