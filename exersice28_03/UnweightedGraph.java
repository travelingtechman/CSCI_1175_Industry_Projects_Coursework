...
public class UnweightedGraph<V> implements Graph<V> {
	...
	public SearchTree dfsNonRecurs(int v) {
		...
		while (!stack.isEmpty()) {
			int x = stack.peek(); //			peek a vertex from the stack, call x
			if(neighbors.get(x).size() == 0) { // if (neighbors list for x equals size 0) {
				stack.pop();//			 	       pop a vertex from the stack
				continue;
			} else {
				for(int i = neighbors.get(x).size()-1; i >= 0; i--) {//	  for (int i = all the vertices in x's neighbor list) {
